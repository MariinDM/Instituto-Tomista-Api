...
export default class UsersController {
  ...
  public async get_Quantity({  response }: HttpContextContract) {
   try {
    const role = await Role.find(1); // Obtener el rol con ID 1
    ...
    return response.status(500).json({ message: 'Error al contar usuarios con rol 1' });
  }
  }
}
